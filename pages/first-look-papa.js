...
export const getStaticProps = async () => {
  const images = await Promise.all(
    files
      .filter((file) => /first-look-papa/.test(file))
      .map(async (file) => {
...
export default function FirstLookPapa({ images }) {
  return (
    <div className="bg-gray-100">
      <div className="flex items-center justify-between py-2 px-4 bg-green-50 opacity-90 sticky top-0 left-0 z-10">
        <div className="flex gap-4 items-center">
          <h1 className="text-2xl font-bold">First Look Papa</h1>
          <DownloadAllButton path="first-look-papa" label="first-look-papa" />
        </div>
